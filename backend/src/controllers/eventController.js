...
const predefinedLocations = [
    { name: "Central Park - Great Lawn", address: "Mid-Park between 79th and 85th Streets, New York, NY", latitude: 40.7812, longitude: -73.9665, isPredefined: true },
    { name: "Prospect Park - Long Meadow", address: "Prospect Park West, Brooklyn, NY", latitude: 40.6602, longitude: -73.9690, isPredefined: true },
    { name: "Washington Square Park", address: "5 Ave, Waverly Pl, W 4th St and Macdougal St, New York, NY", latitude: 40.7308, longitude: -73.9973, isPredefined: true },
    { name: "Union Square Park", address: "E 14th St & Broadway, New York, NY", latitude: 40.7359, longitude: -73.9905, isPredefined: true },
    { name: "Bryant Park", address: "New York, NY 10018 (Between 5th and 6th Avenues, and 40th and 42nd Streets)", latitude: 40.7536, longitude: -73.9832, isPredefined: true }
];
...
exports.createEvent = async (req, res) => {
    try {
        const { title, description, startTime, endTime, locationName, address, latitude, longitude, isPredefined } = req.body;

        if (!title || !description || !startTime || !locationName) {
            return res.status(400).json({ message: 'Missing required event fields (title, description, startTime, locationName).' });
        }
        
        // Validate if a predefined location is chosen, its details should match or be provided
        let eventLocation = { locationName, address, latitude, longitude, isPredefined: !!isPredefined };
        ...
        if (isPredefined && locationName) {
            ...
            if (chosenLocation) {
                ...
                // Or, allow custom 'predefined-like' entries if coords are given
                 return res.status(400).json({ message: 'Selected predefined location not found or missing coordinates.' });
            }
